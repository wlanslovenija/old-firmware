...
static int wlc_bssid(wlc_param param, void *data, void *value)
{
  int ret = -1;
  ...
  if ((param & PARAM_MODE) == GET) {
    ret = wl_iovar_get(interface, "bssid", &bssid, sizeof(bssid));

    if (ret)
      ret = wl_ioctl(interface, WLC_GET_BSSID, &bssid, sizeof(bssid));

    if (!ret) {
      my_ether_ntoa(&bssid, dest);
    }
  } else if ((param & PARAM_MODE) == SET) {
    bssid = *ether_aton(value);
    ret = wl_iovar_set(interface, "bssid", &bssid, sizeof(bssid));
    ret = wl_ioctl(interface, WLC_SET_BSSID, &bssid, sizeof(bssid));
  }
